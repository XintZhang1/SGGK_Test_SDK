...
- `target_api`: an API id from `interface_capabilities.json`. Unknown API ids
  are valid at intake but must produce `needs_harness_extension` rather than an
  invented runnable recipe.
